...
void Port_D_Init(void){
  ...
  // ---------------  Initialize PD3,1,0 as SSI1 MOSI, FS & SCK, PD2 as MISO  ---------------- NOTE PD1 IS SOFTWARE CS
    
  GPIO_PORTD_AMSEL_R    &= ~0x0F;           // disable analog functionality on PD
  GPIO_PORTD_AFSEL_R    |=  0x0D;           // enable alt funct on PD3,0,2
  GPIO_PORTD_DEN_R      |=  0x0F;           // enable digital I/O on PD3,1,0,2
	GPIO_PORTD_DIR_R			|= 	0x02;						// PD1 Output
	
  GPIO_PORTD_PCTL_R      = (GPIO_PORTD_PCTL_R		// SSI1 function for PD3,2,0
                          & 0xFFFF00F0)
                          + 0x00002202;
}
...
void Port_E_Init(void){
 ...
 // ---------------  Initialize PE3,1,0 as GPIO  ---------------------------------  
  ...
  GPIO_PORTE_DIR_R      |=  0x02;           // output digital I/O on PE1
  GPIO_PORTE_DIR_R      &= ~0x09;           // input digital I/O on PE0, PE3
